...
- 英文・日本語訳は文字数によってカードの高さが変わり、それより下の要素の位置が英文ごとにずれてしまうため、他の操作UIより下（末尾）に配置する。画面上から順に以下の並びとする:
  ...
  2. 出題範囲内の総数と現在のインデックス（例:「1/560」。この数字は英文の通し番号ではなく、出題範囲内での再生順の位置を表す。ランダム再生であっても、次へで必ず1増え、前へで必ず1減る。詳細は8.2節・8.3節を参照）、出題順序モード（ランダム再生スイッチ／1リピート再生スイッチ、それぞれON/OFF）、お気に入りのみ表示チェックボックス（ON時は出題範囲内のお気に入りのみを再生対象とする。練習画面固有の状態で、英文選択画面とは共有しない）
  3. プログレスバー
  4. 再生コントロール（前へ／再生／停止／次へ）
  5. 現在の英文のカテゴリ
  6. 現在の英文の通し番号、お気に入り登録／解除ボタン
