...
### 4.0 画面・ラベルの英語表記（UIの英語化）

英語の練習アプリであるため、UIの文言を段階的に英語化している。本書の各節は、画面を指す名称として従来どおり日本語（「練習画面」「英文選択画面」「練習履歴画面」）を用いるが、画面上の表記は次のとおりとする。

| 画面（本書での名称） | 下部タブのラベル／画面の見出し                                                          |
| -------------------- | --------------------------------------------------------------------------------------- |
| 練習画面             | Practice（見出しは置かず、Repeating／Shadowingのモード切り替えが見出しを兼ねる。4.1節） |
| 英文選択画面         | Sentences                                                                               |
| 練習履歴画面         | History                                                                                 |

- 練習履歴画面のセクション見出し: 「Last 7 Days」（直近7日間の棒グラフ）、「Last 28 Weeks」（直近196日間のヒートマップ。196日＝28週）、「All Sentences」（全英文の頻度グリッド）
- 連続学習日数は、練習画面・練習履歴画面ともヘッダー左上に「n-Day Streak」（例:「3-Day Streak」）で表示する（共通コンポーネント`StreakBadge`）
- 練習モードの表記は「Repeating」「Shadowing」で統一する（練習画面のモード切り替え、英文選択画面の英文カードの回数表示「Repeating: n / Shadowing: m」〈「回」は省略〉、棒グラフの凡例）
- 棒グラフの曜日は、アルファベット3文字（Sun／Mon／Tue／Wed／Thu／Fri／Sat）で表示する
- 再生コントロールのラベルは記号（4.1節）
- **暫定的に日本語のまま残す文言**: 「お気に入りのみ表示」「カテゴリ n」。そのほか未対応の文言は`docs/implementation-plan.md`の「追加改修4」のToDoを参照

### 4.1 練習画面（タブ1・初期表示）

- 英文選択画面で選択した出題範囲のうち、現在の英文を表示する
- 英文・日本語訳は文字数によってカードの高さが変わり、それより下の要素の位置が英文ごとにずれてしまうため、他の操作UIより下（末尾）に配置する。画面上から順に以下の並びとする:
  1. 練習モード（Repeating／Shadowing）切り替え。ヘッダー内の見出し（`h1`）を兼ねるラジオボタン（`<input type="radio">`）で、「練習」という見出し文言は置かない。ラジオボタン自体は視覚的にのみ隠し（キーボード操作・スクリーンリーダーでの読み上げは維持する）、選択中の`label`に3px程度の下線を引いてどちらが選ばれているかを示す。見出しは中央寄せにする
  2. 出題範囲内の総数と現在のインデックス（例:「1/560」。この数字は英文の通し番号ではなく、出題範囲内での再生順の位置を表す。ランダム再生であっても、次へで必ず1増え、前へで必ず1減る。詳細は8.2節・8.3節を参照）、出題順序モード（ランダム再生スイッチ／1リピート再生スイッチ、それぞれON/OFF。SVGアイコンで表示し、ON/OFFはアイコンの色で区別する。詳細は下記参照）、お気に入りのみ表示チェックボックス（ON時は出題範囲内のお気に入りのみを再生対象とする。練習画面固有の状態で、英文選択画面とは共有しない）
  3. プログレスバー
  4. 再生コントロール（前へ／再生／停止／次へ）。ボタンのラベルは記号で表示する（前へ=＜、再生=▶、停止=■、次へ=＞）。スクリーンリーダー向けの名前は`aria-label`で英語（Previous／Play／Stop／Next）を付ける。▶は環境によってカラーの絵文字として描画されるため、異体字選択子（U+FE0E）を付けて文字として描画させる
  5. 現在の英文のカテゴリ、通し番号、お気に入り登録／解除ボタン（SVGアイコン〈しおり〉で表示し、登録中はアイコンの色で区別する。英文選択画面〈4.2節〉のお気に入りボタンと共通）。この順に1行に並べる（例:「カテゴリ 1　#1　［しおり］」）
  6. 英文／日本語訳（それぞれ独立にON/OFF切替可能。切替ボタンは目のアイコン〈パスワード表示・非表示のような見た目〉とし、英文・日本語訳それぞれの左側に配置する。非表示にする際はDOMから取り除く（`display: none`相当）のではなく`visibility: hidden`にし、カードの高さ（レイアウト）が変わらないようにする）
- ランダム再生・1リピート再生スイッチはチェックボックス（`<input type="checkbox">`）をDOM上は維持しつつCSSで視覚的にのみ隠し、隣接するSVGアイコンの色をチェックボックスの`checked`疑似クラスで切り替えることでON/OFFを表現する（チェックボックスとしてのフォーカス移動・クリック領域・スクリーンリーダーでの読み上げは維持したまま、見た目だけをアイコンに置き換える）
- 連続学習日数は、ヘッダーの左上に「n-Day Streak」の英語表記（例:「3-Day Streak」）で表示する。狭い画面でも見出し（モード切り替え）と重ならないよう、ヘッダーは2段構成（上段=連続学習日数、下段=中央にモード切り替え）とする
- 英文再生中（`playing`）・待機中（`waiting`）は、タブナビゲーション（英文選択タブ／練習履歴タブ）をdisabledにし、他画面へ遷移できないようにする（8.1節の状態と対応）

### 4.2 英文選択画面（タブ2）

- ヘッダー: 左に、全カテゴリを対象に一括で全選択／全解除するチェックボックスと「選択中の英文数／総数」（例:「200/560」）、中央に画面見出し「英文選択」、右に「設定」ボタンを配置する（3カラム。見出しは画面中央に寄せ、左右の要素と重ならないようにする）。ヘッダーは`position: sticky`で上部に固定し、スクロール中も常に見えるようにする
  - 全選択チェックボックスの左端は、カテゴリ見出し内のチェックボックス・英文カードのチェックボックスの左端と縦方向に揃える
- カテゴリ（セクション番号）ごとに見出しを設け、配下に英文カードを表示する
  - カテゴリの並び順はカテゴリ名の文字列ソートではなく、英文の通し番号（id）順とする（カテゴリ名がゼロ埋めされていない場合、文字列ソートだと「1, 10, 11, ..., 2, 20, ...」のような直感に反する順序になるため）
  - 見出しと配下の英文カードは開閉式のグループとし、見出しクリックで配下の表示・非表示を切り替える（既定は折りたたみ状態。開閉状態は画面をまたいで永続化しない）
  - カテゴリ見出しの表示: カテゴリ名、そのカテゴリ内を全選択／全解除するチェックボックス（開閉トグルとは独立した操作）、「そのカテゴリ内で選択中の英文数／総数」（例:「5/10」）
  - カテゴリを開いている間、そのカテゴリ見出しも`position: sticky`で固定し、次のカテゴリ見出しがスクロールで追いついてくるまで画面上に留まるようにする（ヘッダーの下にスタックする）
  - 固定表示中（本来の位置からずれている状態）のカテゴリ見出しを押して閉じたときは、その見出しが固定位置に留まるようスクロール位置を合わせる（何もしないと、閉じて短くなった分だけ後ろのカテゴリが画面上部に来てしまうため）。固定されていない見出しを閉じるとき、およびカテゴリを開くときはスクロール位置を動かさない
  - 英文カード: 通し番号、英文テキスト、Repeating／Shadowingの回数（「Repeating: n / Shadowing: m」の表記。「回」は省略）、お気に入りボタン、練習対象チェックボックス
- 画面右上「設定」ボタン → [設定画面]（4.2.1節）

#### 4.2.1 設定画面

- Google DriveフォルダID（またはフォルダURL）の入力欄、保存ボタン（詳細は7.2節参照）
- 「同期」ボタン → Drive再取得
...
- ヘッダー: 左上に連続学習日数「n-Day Streak」（4.1節の練習画面と共通の値・共通の表示）、その下段中央に見出し「History」を置く（ヘッダーは練習画面と同じ2段構成）
- 「Last 7 Days」: 直近7日間の棒グラフ（日ごとのRepeating／Shadowing練習回数。凡例はRepeating／Shadowing、曜日はSun〜Sat）
- 「Last 28 Weeks」: 直近196日間（7×28マス）の練習頻度ヒートマップグリッド（GitHubのコントリビューショングラフ風、1マス＝1日）
- 「All Sentences」: 全英文（560マス）の練習頻度グリッド（1マス＝1コンテンツ）
...
- 入力欄はフォルダID単体だけでなく、GoogleドライブのフォルダURL（例: `https://drive.google.com/drive/folders/<フォルダID>`、末尾にクエリパラメータが付く場合や`https://drive.google.com/open?id=<フォルダID>`形式を含む）の入力も受け付ける。URL形式が入力された場合はフォルダID部分を自動抽出して以降の処理に用いる（スマートフォンではURLのコピーは容易だがID部分のみの抽出は手間であるため）。上記いずれのパターンにも一致しない入力は、フォルダIDそのものとみなしてそのまま扱う
- 入力値（抽出後のフォルダID）は`localStorage`（`ondoku:driveSettings`）に保存する（公開リポジトリのソースコードには含めない）
...
- 出題範囲が0件になった場合（全チェックOFF、またはお気に入りが1件もない状態でfavoritesOnly=ONにした場合等）、練習画面のUI自体は維持したまま（練習モード切替・お気に入りのみ表示チェックボックス等は操作可能なまま）、英文を表示する箇所に「練習対象の英文が選択されていません」という案内メッセージを表示し、再生・前へ／次へ／お気に入りボタンをdisabledにする。特に「お気に入りのみ表示」チェックボックスは無効化せず、ユーザーがそこから直接チェックを外して復帰できるようにする（英文選択画面に移動しなくても回復できるようにするため）
  - 実装上の注意: 出題範囲が非空→空に変化した際、`usePlaybackEngine`内部の現在コンテンツIDは（停止中に限りリセットする設計のため）直前の値のまま残ることがある。そのIDが「コンテンツ自体は存在する」というだけで案内メッセージを出し忘れないよう、現在のコンテンツが実際に出題範囲（プレイリスト）に含まれているかどうかで判定すること。あわせて`domain/playback`の状態遷移ロジックも、出題範囲が0件の間はあらゆるイベント（`PLAY`/`AUDIO_ENDED`/`WAIT_ENDED`/`NEXT`/`PREV`）を安全に無視し、`playlist[0]`（`undefined`）を現在のコンテンツIDにしてしまわないようにする（`undefined`のIDでIndexedDBへアクセスするとエラーになるため）
...
再生順は「出題範囲（プレイリスト）全件を1周する順序（`playOrder`）」と「その中での現在位置（`roundPosition`。0始まり。画面表示の「n/総数」は`roundPosition + 1`）」で管理する。`isRandom === false`（順次）のときは`playOrder`は出題範囲そのもの（通し番号順）、`isRandom === true`（ランダム）のときはFisher-Yatesシャッフルした並びとなる。出題範囲の変更（英文選択の変更・お気に入りのみ表示の切替等）や`isRandom`のON/OFF切替を検知すると、その時点で`playOrder`を再構築する（順次再生への切替時は通し番号順に、ランダム再生への切替時は再シャッフルする）。

`playing`→（再生終了）または`waiting`→（待機終了）した際の挙動は、`OrderSettings`の2つのスイッチで決まる:

1. `isRepeatOne === true`の場合: `roundPosition`を更新せず、同じコンテンツを対象として`playing`へ戻る（8.1節の状態遷移図の通り）
2. `isRepeatOne === false`の場合: `roundPosition`を1つ進めてから`playing`へ遷移する
   - `roundPosition`が`playOrder`の末尾に達していない場合: 単純に1つ進める（`isRandom`のON/OFFに関わらず、選択した英文がすべて出題されるまで、既に出題済みの英文は選ばれない）
   - `roundPosition`が`playOrder`の末尾に達した場合（出題範囲を1周し終えた場合）: `roundPosition`を0に戻し、`isRandom === true`のときのみ`playOrder`を再シャッフルして次のラウンドに入る（直前に再生したコンテンツが新ラウンドの先頭に来ないようにする）。`isRandom === false`のときは同じ`playOrder`（通し番号順）を繰り返す

### 8.3 次へ／前へボタンの挙動

次へ／前へボタンは、`isRandom`・`isRepeatOne`のON/OFFの組み合わせに関わらず共通のルールで動作する。「次に進む先のコンテンツ」の決め方は8.2節の進行ロジックと同一（`isRepeatOne`のON/OFFはこのボタン操作には影響しない＝ボタンを押せば1リピート再生中でも次・前のコンテンツに移動する）:

- 現在の状態が`playing`または`waiting`のとき: 直ちに現在の再生・待機をキャンセルし、次／前のコンテンツに切り替えて`playing`へ遷移する（自動再生を継続）
- 現在の状態が`stopped`のとき: 次／前のコンテンツへ`roundPosition`のみ変更する（自動再生はしない。ユーザーが改めて再生ボタンを押すまで`stopped`のまま）
- 「次へ」は8.2節の進行ロジックと同一（`roundPosition`を1つ進め、ラウンド末尾に達した場合のみ`isRandom === true`なら再シャッフルする）
- 「前へ」は`isRandom`のON/OFFに関わらず常に有効で、`roundPosition`を1つ戻す（ラウンド先頭で「前へ」を押した場合は、再シャッフルせず現在の`playOrder`内で末尾に循環する）。表示上の数字は、次へ／前へどちらの操作でも必ず1ずつ増減する
...
- 20行×28列のグリッドで、行優先（左上から右へ進み、右端で折り返して次の行へ）に配置する。マスの大きさは固定値ではなく表示領域の横幅から計算する（スマートフォンの画面幅に応じて可変）。9.4節の日別ヒートマップグリッド（7行×28列）と列数を揃えることで、2つのグリッドが同じ大きさで表示されるようにする
...
- **直近196日間のヒートマップグリッド**: 7行（曜日）×28列（週）のグリッドで、1マス＝1日、列は週単位（GitHubのコントリビューショングラフと同様の並び）とする。マスの色の濃淡は、その日の合計練習回数（`repeatingCount + shadowingCount`）を9.3節と同様の区分で段階分けして決定する（マスの大きさは固定値ではなく表示領域の横幅から計算し、列数は9.3節の頻度グリッド〈20行×28列〉と揃えることで、2つのグリッドが同じ大きさで表示されるようにする）
