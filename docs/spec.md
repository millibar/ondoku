...
- 出題範囲は `PracticeSessionState.selectedContentIds`（英文選択画面の練習対象チェックボックスでONの英文ID一覧）を基準とする
- `favoritesOnly === true` の場合、さらに `isFavorite === true` の英文のみに絞り込む
- 英文選択画面の「カテゴリ絞り込み」（表示フィルター）は出題範囲に影響しない
- 出題範囲が0件になった場合（全チェックOFF、またはお気に入りが1件もない状態でfavoritesOnly=ONにした場合等）、練習画面は再生不可（プレイリストが空である旨を表示し、再生・次へ／前へボタンはdisabled）とする
...
### 13.1 ページ構成変更（本改訂）で新たに確定させる事項

- 直近7日間棒グラフの見た目（積み上げ棒 or 2本並列棒）
- 出題範囲（`selectedContentIds`）が0件の場合の練習画面UI（メッセージ表示・再生系ボタンのdisabled化の具体的な見せ方）
- 英文選択画面のカテゴリ絞り込み・全選択／全解除チェックボックスのUI詳細（tri-state表示要否など）
- 下部タブナビゲーションのdisabled時の見た目・操作不可であることの伝え方
