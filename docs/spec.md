...
- [設定画面]: Drive接続設定（フォルダID変更）、ログアウト、アプリバージョン表示
- 中断・再開: 練習画面を離れる（アプリを閉じる／一覧に戻る）際に現在の練習状態を`localStorage`に保存し、次回練習画面を開いたときに同じ状態（モード・出題範囲・出題中コンテンツ）を復元する（5.4節参照）
...
- 同じ日に複数回練習した場合も、`DailyLog`にはその日の記録を1件のみ保持する（日付をキーとしたUpsert）

### 9.2 連続学習日数（ストリーク）の算出

- `dailyLogs`ストアの日付一覧から、今日（または直近の練習日が昨日まで）から過去に向かって連続する日数をカウントする（`src/domain/streak.ts`に純粋関数として実装し、Vitestで単体テストする）
...
- GitHub Pagesはリポジトリ名のサブパス配信（例: `https://<user>.github.io/ondoku/`）となるため、Viteの`base`設定・manifestの`start_url`/`scope`をリポジトリ名に合わせる（**リポジトリ名は実装計画書フェーズで確定**。現時点では`package-lock.json`の`name`が`ondoku`であることから、`/ondoku/`を仮定する）

## 11. 非機能仕様

- 対応ブラウザ: スマートフォンのモダンブラウザ（Android Chrome優先）＋ 開発・確認用にPC Chrome
- オフライン時の挙動: Drive未接続時でも、IndexedDBにキャッシュ済みのコンテンツ・音声・履歴を用いて一覧画面・練習画面が利用できる。「同期」ボタンはオフライン時は無効化しエラーメッセージを表示する
- エラーハンドリング方針:
  - Drive API呼び出し失敗（認証切れ・ネットワークエラー・ファイル未検出等）は、画面上にトースト等でメッセージ表示し、アプリ全体をクラッシュさせない
  - 音声再生エラー（未対応フォーマット等）は該当コンテンツをスキップ可能にし、エラーであることをコンテンツ一覧上でも分かるようにする（詳細はUI実装時に検討）
...
- **単体テスト（Vitest）**: `src/domain/`配下の純粋ロジック（TSVパーサー、再生状態遷移・進行ロジック、ストリーク計算、頻度グリッドの色区分計算）を中心に、Reactに依存しない形でテストする
