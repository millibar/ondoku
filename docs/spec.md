...
- 英文・日本語訳は文字数によってカードの高さが変わり、それより下の要素の位置が英文ごとにずれてしまうため、他の操作UIより下（末尾）に配置する。画面上から順に以下の並びとする:
  ...
  5. 現在の英文のカテゴリ
  6. 現在の英文の通し番号、お気に入り登録／解除ボタン（SVGアイコン〈しおり〉で表示し、登録中はアイコンの色で区別する。英文選択画面〈4.2節〉のお気に入りボタンと共通）
  7. 英文／日本語訳（それぞれ独立にON/OFF切替可能。切替ボタンは目のアイコン〈パスワード表示・非表示のような見た目〉とし、英文・日本語訳それぞれの左側に配置する。非表示にする際はDOMから取り除く（`display: none`相当）のではなく`visibility: hidden`にし、カードの高さ（レイアウト）が変わらないようにする）
