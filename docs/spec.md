...
- 20行×28列のグリッドで、行優先（左上から右へ進み、右端で折り返して次の行へ）に配置する。9.4節の日別ヒートマップグリッド（7行×28列）と列数・マスサイズを揃え、2つのグリッドが同じ大きさで表示されるようにする
...
- **直近196日間のヒートマップグリッド**: 7行（曜日）×28列（週）のグリッドで、1マス＝1日、列は週単位（GitHubのコントリビューショングラフと同様の並び）とする。マスの色の濃淡は、その日の合計練習回数（`repeatingCount + shadowingCount`）を9.3節と同様の区分で段階分けして決定する（列数・マスサイズは9.3節の頻度グリッド〈20行×28列〉と揃え、2つのグリッドが同じ大きさで表示されるようにする）
