...
- 画面上部: 全カテゴリを対象に一括で全選択／全解除するチェックボックスと、「選択中の英文数／総数」（例:「200/560」）。`position: sticky`でヘッダー直下に固定し、スクロール中も常に見えるようにする
- カテゴリ（セクション番号）ごとに見出しを設け、配下に英文カードを表示する
  ...
  - カテゴリを開いている間、そのカテゴリ見出しも`position: sticky`で固定し、次のカテゴリ見出しがスクロールで追いついてくるまで画面上に留まるようにする（ヘッダー・上部サマリーの下にスタックする）
