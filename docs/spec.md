...
│   │   ├── WeeklyBarChart.tsx  # 今週（日曜始まり）の棒グラフ
│   │   ├── ProgressBar.tsx
...
| 英文選択画面         | Sentences                                                                               |
| 練習履歴画面         | History                                                                                 |
| 設定画面             | Settings（見出しは中央寄せ）                                                            |

- 練習履歴画面のセクション見出し: 「This Week」（今週〈日曜始まり〉の棒グラフ）、「Last 28 Weeks」（直近196日間のヒートマップ。196日＝28週）、「All Sentences」（全英文の頻度グリッド）
...
- カテゴリの表示名は教材ごとに異なる（例: DUO 3.0は「SECTION」、英語復文勉強法は「Chapter」）ため、固定の文言にせず、教材TSVの2列目のヘッダーを表示名として使う（6章）。練習画面・英文選択画面では「<表示名> <カテゴリ番号>」（例:「SECTION 1」）と表示する。ヘッダーが使えない場合の表示名は「Category」
- **暫定的に日本語のまま残す文言**: 「お気に入りのみ表示」。そのほか未対応の文言は`docs/implementation-plan.md`の「追加改修4」のToDoを参照
...
- 英文・日本語訳は文字数によってカードの高さが変わり、それより下の要素の位置が英文ごとにずれてしまうため、他の操作UIより下（末尾）に配置する。画面上から順に以下の並びとする:
  ...
  5. 現在の英文のカテゴリ、通し番号、お気に入り登録／解除ボタン（SVGアイコン〈しおり〉で表示し、登録中はアイコンの色で区別する。英文選択画面〈4.2節〉のお気に入りボタンと共通）。この順に1行に並べる（例:「SECTION 1　#1　［しおり］」。カテゴリの表示名は4.0節）
...
- ヘッダー: 左に、全カテゴリを対象に一括で全選択／全解除するチェックボックスと「選択中の英文数／総数」（例:「200/560」）、中央に画面見出し「英文選択」、右に設定ボタン（歯車のSVGアイコン。スクリーンリーダー向けの名前は`aria-label`で「Settings」）を配置する（3カラム。見出しは画面中央に寄せ、左右の要素と重ならないようにする）。ヘッダーは`position: sticky`で上部に固定し、スクロール中も常に見えるようにする
...
- カテゴリ（セクション番号）ごとに見出しを設け、配下に英文カードを表示する
  ...
  - カテゴリ見出しの表示: カテゴリ名（「<表示名> <カテゴリ番号>」。表示名は4.0節）、そのカテゴリ内を全選択／全解除するチェックボックス（開閉トグルとは独立した操作）、「そのカテゴリ内で選択中の英文数／総数」（例:「5/10」）
...
- 画面右上の設定ボタン → [設定画面]（4.2.1節）

#### 4.2.1 設定画面

- ヘッダー: 左に閉じるボタン（＜を○で囲んだSVGアイコン。スクリーンリーダー向けの名前は`aria-label`で「閉じる」）、中央に見出し「Settings」を配置する（3カラム。見出しは画面中央に寄せる）
- 現在保存されているGoogle DriveのフォルダIDを表示する（入力欄ではなく表示のみ。未設定の場合は「未設定」）
- 「教材を変更する...」ボタン → 教材（フォルダ）変更のポップアップを開く（詳細は7.2節参照）
  - ポップアップには「Google DriveのフォルダIDまたはURL」の入力欄（空の状態で開く）、「変更して同期」ボタン、「キャンセル」ボタンを表示する
  - 「変更して同期」: ポップアップを閉じ、入力値から抽出したフォルダIDを保存し、そのフォルダからの同期を開始する。入力欄が空の間は押せない
  - 「キャンセル」（またはEscキー）: ポップアップを閉じる。フォルダIDは変更しない
  - フォルダIDの保存と同期を1つの操作にまとめているのは、フォルダを変更したのに同期されず古い教材のまま残る状態や、入力しただけで保存していないIDと同期対象がずれる状態を防ぐため
- 「同期」ボタン → 現在保存されているフォルダからDrive再取得（Drive上の教材を更新したときに使う）。ラベルの頭に同期のSVGアイコン（雲と循環の矢印）を付ける
  - ボタンの上に次の説明文を表示する:「Google Drive上の教材（TSVファイルや音声）を更新した場合、現在のフォルダから教材を取り込み直せます（練習記録・お気に入りは削除されません）。すべての音声をダウンロードし直すため、Wi-Fi環境での実行をおすすめします。」（11章のWi-Fi推奨の案内を兼ねる）
- 「キャッシュを更新」ボタン（ラベルの頭に更新のSVGアイコン〈2本の循環の矢印〉を付ける）: Service WorkerのCache Storage（プリキャッシュしたJS/CSS/HTML等のアプリシェル）と登録済みのService Worker自体をすべて削除し、ページを再読み込みする。これにより、PWAとしてキャッシュされた古いアプリの表示から最新のデプロイ内容に切り替えられる。IndexedDB（教材データ・練習履歴）・localStorage（Drive設定・練習状態）は対象外で削除されない
- 閉じるボタン → 英文選択画面に戻る
- ログアウトボタンは置かない。個人の端末で使う想定で必要な場面が無く、また端末内にキャッシュ済みデータがあれば次回起動時はログインを待たずにアプリ本体へ進む（11章）ため、画面をログイン画面に戻すだけのログアウトは実質的に意味を持たないため

### 4.3 練習履歴画面（タブ3）

- ヘッダー: 左上に連続学習日数「n-Day Streak」（4.1節の練習画面と共通の値・共通の表示）、その下段中央に見出し「History」を置く（ヘッダーは練習画面と同じ2段構成）
- 「This Week」: 今週（日曜始まり）の棒グラフ（日ごとのRepeating／Shadowing練習回数。前週の同じ曜日の回数を薄い棒で重ねて表示する。凡例はRepeating／Shadowing／Last Week、曜日はSun〜Sat）
...
// 教材ごとの設定（localStorage）。同期時にTSVのヘッダーから取り出して保存する（6章）
...
| `ondoku:materialSettings`     | `MaterialSettings`     |

> Googleの認証トークン（アクセストークン）はlocalStorageに保存せず、メモリ上でのみ保持する。アプリ再起動時はGISのサイレント再認証（`prompt: ''`）を試み、失敗した場合、キャッシュ済みデータが無ければログイン画面を表示する（キャッシュ済みデータがあればアプリ本体を表示し、同期の操作時に認証する。7.1節）。

## 6. TSVフォーマット仕様

- 区切り文字: タブ（`\t`）
- 文字コード: UTF-8
- 1行目: ヘッダー行（列名）。列の読み取りはヘッダーの値ではなく列順で行う
  - 2列目のヘッダーは、カテゴリの表示名として使う（例: DUO 3.0なら「SECTION」、英語復文勉強法なら「Chapter」）。大文字小文字はそのまま、前後の空白は取り除いて使う
  - 2列目のヘッダーが空、または従来の列名`categoryId`（大文字小文字を区別しない）の場合は、表示名が無いものとして扱い、既定の「Category」を表示する
  - 表示名は同期時に`localStorage`（`ondoku:materialSettings`）に保存し、次回の同期まで使う。同期するまでは既定の「Category」を表示する
  - 2列目以外のヘッダーの値は使わない
- 列構成（要件定義書4.3節に対応）:
...
- トークンはメモリ上にのみ保持する。取得時に受け取る有効期間（`expires_in`。通常1時間）から有効期限を計算して一緒に保持する
- 起動時はサイレント再認証（`prompt: ''`）をバックグラウンドで試みる。ただしiOSのホーム画面アプリ（PWA）などでは、ユーザー操作を伴わない認証がブラウザの制限（ポップアップ・サードパーティCookieの制限）で失敗することが多い。キャッシュ済みデータがある場合は、失敗してもアプリ本体をそのまま使える（11章）
- トークンが必要な操作（設定画面の「同期」、教材変更ポップアップの「変更して同期」）を行ったとき、トークンが無い、または有効期限切れ（残り60秒未満を含む）の場合は、その操作（タップ）の中でGISの認証（`prompt: ''`。初回以外は同意画面を出さずアカウント選択のみで済む）を開始し、トークンを取得してから同期する。ユーザー操作の中で開始することで、iOSでもポップアップが制限されにくくする
  - この認証に失敗・キャンセルした場合は、同期を行わず「Googleへのログインに失敗しました。もう一度お試しください。」を表示する
...
- 初回起動時、初期セットアップ画面で「ルートフォルダID」をユーザーが入力する。設定後の変更は、設定画面の「教材を変更する...」ポップアップで行う（4.2.1節）
...
- **今週の棒グラフ**: 今日を含む週（日曜始まり）の日曜〜土曜の`DailyLog`（無い日は0件扱い）を曜日順に並べ、日ごとに`repeatingCount`・`shadowingCount`を表示する。曜日の位置は固定で、左端が常に日曜日になる
  - 前週（今週の日曜の7日前〜前日の土曜）の同じ曜日の回数を、同じ位置に薄い色の棒で重ねて表示する（今週の棒が手前）。前週と比較できるようにするため
  - 今日より後の曜日（今週のまだ来ていない日）は、今週の棒を表示せず前週の棒のみを表示する
  - 棒の高さは、今週・前週の全ての棒の最大値を100%とした相対値とする
  - 日付の算出（今週・前週の日付と、未来の日かどうか）は`src/domain/dailyGrid.ts`に純粋関数として実装する
...
- 今週の棒グラフはグループ化棒グラフ（日ごとにリピーティング／シャドーイング2本を並べる）とした（9.4節）。のちに日曜始まりの固定とし、前週の棒を薄く重ねる表示を追加した
