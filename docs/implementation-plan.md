...
WP6〜WP10により、ページ構成変更（3タブ構成＋設定サブ画面への再編）の実装が完了した。実データでの疎通確認・E2Eシナリオの拡充は、既存のWP5「結合・実データ確認」の範囲で改めて実施する。

## 4.2 ページ構成変更後の追加改修

WP10完了・マージ後にユーザーから寄せられた改善要望・不具合報告への対応。1件＝1ブランチとし、都度ユーザー承認を得て`main`へマージする。

### 追加改修1: 出題対象0件時の練習画面UI維持（`fix/practice-empty-playlist-ui`）

- 出題対象が0件でも練習画面のUI・お気に入りのみ表示チェックボックスは維持し、案内メッセージ＋再生系ボタンのdisabledで表現するよう変更（仕様書8.0節）
- 実機確認で追加発見: `usePlaybackEngine`がplaylistの変化に反応せず、内部状態が固定されたまま復帰できない不具合を修正（停止中のみ先頭にリセット）
- ✅ 完了・ユーザー承認を得て`main`へマージ済み（2026-08-30）

### 追加改修2: 英文選択画面のカテゴリ折りたたみ表示・全選択サマリー（`feature/selection-screen-collapsible-categories`）

- カテゴリごとの表示フィルター（廃止）→ カテゴリ見出しクリックによる開閉式グループ表示に変更
- カテゴリ見出しに「選択中/総数」を表示
- 画面上部に全カテゴリ一括選択チェックボックスと「選択中/総数」表示を追加（`onToggleAllSelection`を新設）
- **完了条件**: `ContentSelectionScreen`のコンポーネントテストが成功、既存テスト・lint・format・buildがすべて成功すること
...
### 追加改修4: UIの英語化（段階的に実施。`feature/english-ui-playback-symbols`）

英語の練習アプリであるため、UIの文言を英語化する。一度に全部は行わず、影響の大きい箇所から段階的に進める。画面上の表記は仕様書4.0節にまとめる。

- **状況（第1段階）**: ✅ 実装済み（コミットはブランチ上。`main`へのマージはユーザー承認待ち）
  - 再生コントロールのラベルを記号（＜ ▶ ■ ＞）に変更。スクリーンリーダー向けの`aria-label`は英語（Previous／Play／Stop／Next）。▶には異体字選択子（U+FE0E）を付ける
  - 下部タブと画面見出し: 練習→**Practice**、英文選択→**Sentences**、練習履歴→**History**
  - 練習履歴画面のセクション見出し: 直近7日間→**Last 7 Days**、直近196日間→**Last 28 Weeks**、全英文→**All Sentences**
  - 連続学習日数を「**n-Day Streak**」の英語表記にし、練習履歴画面もヘッダー左上に配置（練習画面と共通の`StreakBadge`コンポーネントに切り出し）
  - 棒グラフの凡例を**Repeating／Shadowing**、曜日を**Sun〜Sat**に変更
  - 英文選択画面の英文カードの回数表示を「**Repeating: n / Shadowing: m**」に変更（「回」は省略）
- **暫定的に日本語のまま残す（ユーザー指示）**: 「お気に入りのみ表示」「カテゴリ n」（練習画面・英文選択画面）

#### ToDo（第2段階以降。英語化が必要と思われる残りの箇所）

方針: 表記を英語にそろえる際は、視覚的な文言だけでなく、`aria-label`（スクリーンリーダー向けの名前）・`title`・エラーメッセージも対象とする。文言を変えた箇所は、対応するテスト・仕様書4.0節・テスト計画書も更新する。

- [ ] **練習画面**（`PracticeScreen.tsx`）
  - アイコンの読み上げ名: 「ランダム再生」「1リピート再生」
  - 練習モードのラジオグループの`aria-label`「練習モード」
  - お気に入りボタンの`aria-label`「お気に入りに追加」「お気に入りから解除」（英文選択画面と共通）
  - 出題対象0件時の案内メッセージ。文中で「英文選択画面」に言及しているが、タブ名は**Sentences**に変更済みのため、**表記が食い違っている**（優先して直す）。「お気に入りのみ表示」への言及は、その文言の扱いが決まるまで保留
- [ ] **英文選択画面**（`ContentSelectionScreen.tsx`）
  - チェックボックスの`aria-label`: 「すべて選択」「カテゴリnを全選択」「#nを練習対象にする」
  - お気に入りボタンの`aria-label`（上記と共通）
- [ ] **練習履歴画面**
  - 棒グラフの各棒の`aria-label`（「日付 リピーティング n回」「日付 シャドーイング n回」）→ Repeating／Shadowing、「回」の扱いを決める（`WeeklyBarChart.tsx`）
  - ヒートマップの`aria-label`「日別練習頻度ヒートマップ」と各マスの`title`「日付: n回」（`DailyHeatmapGrid.tsx`）
  - 頻度グリッドの`aria-label`「学習頻度グリッド」（`FrequencyGrid.tsx`）
- [ ] **英文・日本語訳の表示切り替え**（`ContentText.tsx`）: 目のアイコンの`aria-label`「英文を表示／隠す」「日本語訳を表示／隠す」。ラベル「日本語訳」の英語名（例: Japanese）を決める
- [ ] **下部タブナビゲーション**（`BottomTabNav.tsx`）: `nav`の`aria-label`「画面切り替え」
- [ ] **設定画面**（`SettingsScreen.tsx`）: 「閉じる」「保存」「同期」「キャッシュを更新」、フォルダID入力のラベル、キャッシュ更新の説明文
- [ ] **ログイン画面**（`LoginScreen.tsx`）: 見出し「英語音読練習」（アプリ名の英語表記を決める）、説明文、「Googleでログイン」
- [ ] **初期設定画面**（`SetupScreen.tsx`）: 見出し「初期設定」、説明文、フォルダID入力のラベル、「次へ」
- [ ] **アプリ全体の状態表示・エラーメッセージ**（`App.tsx`）: 「読み込み中...」「同期中...」「音声を準備中...」、同期失敗・ログイン失敗・Driveフォルダ未設定のメッセージ。`console.error`のメッセージは開発者向けのため、対象とするかは要判断
- [ ] **アプリ名・メタ情報**: `index.html`の`<title>`と`lang="ja"`、`vite.config.ts`のPWAマニフェスト（`name`「英語音読練習」、`short_name`「音読」、`description`、`lang`）。アプリ名の英語表記を決めてから変更する
- [ ] **「お気に入りのみ表示」「カテゴリ n」**: 現在は暫定的に日本語のまま。方針が決まったら対応する（練習画面・英文選択画面の両方。関連するテスト・案内メッセージも含む）
- [ ] **文言の扱いの確認事項**
  - 英語化した画面と、日本語のまま残る画面が混在する期間の見え方（特に設定・ログイン系の画面）
  - 英文の日本語訳そのもの（教材データ）は英語化の対象外
